...
<head>
    <title>{{__('Liste des pîliers') }} || Adisa</title>
                    ...
                    <div class="toolbar" id="kt_toolbar">
                        <div class="container-fluid d-flex flex-stack flex-wrap flex-sm-nowrap">
                            ...
                            <div class="d-flex flex-column align-items-start justify-content-center flex-wrap me-2">
                                <!--begin::Title-->
                                <h1 class="text-dark fw-bolder my-1 fs-2">{{__('Liste des pîliers') }}</h1>
                                <!--end::Title-->
                                <!--begin::Breadcrumb-->
                                <ul class="breadcrumb fw-bold fs-base my-1">
                                    ...
                                    <li class="breadcrumb-item text-muted"><a href="{{ route('piliers-list') }}" class="text-muted text-hover-primary">{{__('Piliers et cours') }}</a></li>
                                    <li class="breadcrumb-item text-muted">{{__('Liste des pîliers') }}</li>
                                    ...
                                    <div class="card-toolbar">
                                        ...
                                        <div class="me-4 my-1">
                                            <select id="kt_filter_orders" name="orders" data-control="select2" data-hide-search="true" class="w-125px form-select form-select-sm form-select-solid">
                                                <option value="All" selected="selected">{{__('Tout') }}</option>
                                                <option value="Approved">{{__('Actif') }}</option>
                                                <option value="Declined">{{__('Desactivé') }}</option>
                                            </select>
                                        </div>
                                        <!--end::Select-->
                                        <!--begin::Search-->
                                    ...
                                    <table id="kt_orders_classic" class="table table-row-bordered table-row-dashed g-3 gs-0 align-middle">
                                        <thead class="fs-7 fw-boldest text-gray-400 text-uppercase">
                                            <tr>
                                                ...
                                                <th class="min-w-150px">{{__('Date de création') }}</th>
                                                <th class="min-w-100px">{{__('Nombre de Cours') }}</th>
                                            ...
                                            <tr>
                                                <td>
                                                    ...
                                                    <div class="d-flex">
                                                        ...
                                                        @if (!empty($pilier->image))
                                                            <img src="{{asset($pilier->image)}}" alt="{{$pilier->nom}}" />
                                                            @else
                                                            <p>pas d'image</p>
                                                            @endif
                                                        </div>
                                                        ...
                                                        <div class="d-flex flex-column justify-content-center">
                                                            <a href="#" class="mb-1 text-gray-800 text-hover-primary">{{ $pilier->titre}}</a>
                                                            <div class="fw-bold text-gray-400">{{ $pilier->createur}}</div>
                                                        </div>
                                                        <!--end::Info-->
                                                    </div>
                                                    <!--end::Wrapper-->
                                                </td>
                                                <td>{{ $pilier->created_at}}</td>
                                                <td>faire un count</td>
                                                <td>
                                                    @if($pilier->statut == "actif")
                                                    <span class="badge badge-light-info fw-boldest">{{ $pilier->statut}}</span>
                                                    @else
                                                    <span class="badge badge-light-danger fw-boldest">{{ $pilier->statut}}</span>
                                                    @endif
                                                </td>
                                                <td class="text-end">
                                                    <!-- Button trigger modal -->

                                                    <a href="{{ route('piliersview', $pilier->id) }}" class="btn btn-light btn-sm">{{__('Voir') }}</a>
                                                    <a href="{{ route('piliersedit', $pilier->id) }}" class="btn btn-light btn-sm">{{__('Editer') }}</a>
                                                </td>
                                            </tr>
                                            @endforeach
                                        </tbody>
